...
const useControlledForm = (props: IUseFormSettings): UseControlledForm => {
    ...
    const onChange = useCallback((v: IValues) => {
...
describe("useControlledForm", () => {
    ...
    beforeAll(async () => {
        ...
        renderHookResults = await renderHook(
            (config = FORM_CONFIG) => useControlledForm(config),
            {
    ...
    test("set values by external state", async () => {
        const { result, act, rerender } = renderHookResults
    ...
    test("set values by internal methods", async () => {
        ...
        await act(() =>
            result.current[2].setValue("username", "some_user_name_2")
        )
        await act(() =>
            result.current[2].setValue("password", "some_password_2")
        )
    ...
    test("validate values without any error", async () => {
        ...
        await act(() => result.current[2].setTouchedByName("username"))
        await act(() => result.current[2].setTouchedByName("password"))
    ...
    test("send form error", async () => {
        ...
        await act(async () => {
            ...
            ).rejects.toThrow("Request has failed")
        })
